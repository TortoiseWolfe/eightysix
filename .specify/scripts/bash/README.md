...
| `config.sh`               | eightysix-specific   | sourced by `common.sh` (top of file)                             |
...
| `update-agent-context.sh` | eightysix stub       | `/speckit.plan` Phase 1 step 3                                   |

Upstream: <https://github.com/github/spec-kit/tree/main/scripts/bash>

## eightysix adaptations

Upstream SpecKit assumes flat `specs/<NNN-name>/` layout. eightysix uses
...
`update-agent-context.sh` is a deliberate no-op: eightysix's `CLAUDE.md` is
...
Then re-apply the eightysix adaptations listed above. The patches are small
(~50 lines total) and the diffs in git history show what to re-apply.

The `config.sh` and `update-agent-context.sh` files are eightysix-only — never
