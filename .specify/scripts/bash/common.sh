...
#
# eightysix adaptation: this is vendored from upstream github/spec-kit.
...
#   - get_current_branch() (non-git fallback) — same recursion
...
# Source eightysix-specific config (SPEC_KIT_FEATURES_ROOT, etc.)
...
get_current_branch() {
    ...
    # eightysix adaptation: recurse through features/<category>/* instead of flat specs/*.
    ...
    if [[ -d "$features_root" ]]; then
        ...
        # Two-level glob: features/<category>/<NNN-name>/. Falls through to flat
        # features/<NNN-name>/ via the same loop because the inner glob expands too.
        for dir in "$features_root"/*/* "$features_root"/*; do
            if [[ -d "$dir" ]]; then
...
#
# eightysix adaptation: searches recursively under features/<category>/<prefix>-*/
...
find_feature_dir_by_prefix() {
    ...
    local features_root="$repo_root/${SPEC_KIT_FEATURES_ROOT:-specs}"
    local default_category="${SPEC_KIT_DEFAULT_CATEGORY:-_uncategorized}"
    ...
    if [[ "$branch_name" =~ ^([0-9]{8}-[0-9]{6})- ]]; then
        ...
        # under the default category (so downstream errors are still readable).
        echo "$features_root/$default_category/$branch_name"
        return
    fi

    # Recursively search features/<category>/<prefix>-*/ for the prefix.
    # Falsetto for the legacy flat layout features/<prefix>-*/ too, in case
    # someone authored under SPEC_KIT_DEFAULT_CATEGORY=. style.
    local matches=()
    if [[ -d "$features_root" ]]; then
        # nullglob ensures the loop body doesn't execute when a glob matches nothing
        # (default behavior would pass the literal pattern through).
        local nullglob_was_set=false
        shopt -q nullglob && nullglob_was_set=true
        shopt -s nullglob
        for dir in "$features_root"/*/"$prefix"-* "$features_root"/"$prefix"-*; do
            if [[ -d "$dir" ]]; then
                matches+=("$dir")
            fi
        done
    ...
    if [[ ${#matches[@]} -eq 0 ]]; then
        # No match found - return the default-category path (will fail later with clear error)
        echo "$features_root/$default_category/$branch_name"
    elif [[ ${#matches[@]} -eq 1 ]]; then
        # Exactly one match - perfect! Return absolute path including category.
        echo "${matches[0]}"
    else
        # Multiple matches - shouldn't happen with proper naming convention
        echo "ERROR: Multiple feature directories found with prefix '$prefix': ${matches[*]}" >&2
        echo "Please ensure only one feature directory exists per prefix." >&2
        return 1
    fi
}
