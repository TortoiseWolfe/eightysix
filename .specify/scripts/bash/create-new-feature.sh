...
# eightysix adaptation: scans both flat specs_dir/* (upstream layout) and
# nested specs_dir/<category>/* (eightysix layout) so the next-feature-number
...
get_highest_from_specs() {
    ...
    if [ -d "$specs_dir" ]; then
        ...
        shopt -s nullglob
        for dir in "$specs_dir"/* "$specs_dir"/*/*; do
            [ -d "$dir" ] || continue
...
# eightysix adaptation: features live under features/<category>/, not specs/
...
DEFAULT_CATEGORY="${SPEC_KIT_DEFAULT_CATEGORY:-}"
if [ -n "$DEFAULT_CATEGORY" ]; then
    NEW_FEATURE_PARENT="$SPECS_DIR/$DEFAULT_CATEGORY"
else
    NEW_FEATURE_PARENT="$SPECS_DIR"
fi
if [ "$DRY_RUN" != true ]; then
    mkdir -p "$NEW_FEATURE_PARENT"
fi
...
FEATURE_DIR="$NEW_FEATURE_PARENT/$BRANCH_NAME"
