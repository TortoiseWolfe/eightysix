...
- **US3 reframed as "switch payment method (provider switch)"**: the spec's literal "Update Payment Method" (FR-011-FR-015) assumes saved cards + stripe.js Elements + a PCI surface. eightysix never stores cards — every checkout is a fresh Stripe-hosted Checkout (or PayPal redirect, or Cash App / Chime direct link). The honest interpretation in this codebase is "after a card decline, let the user pick a different provider." Implemented via `<SwitchProviderPanel>` in PR #43-followup. Saved-card storage (Stripe Customer + saved_payment_methods table + `<CardElement>`) is a separate multi-PR feature and not appropriate for this template's static-export architecture.
